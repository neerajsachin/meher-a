function needAttention (InDate){
	var threshold = 35;
	return t1 = moment().startOf('day').add(threshold,'days').isAfter(InDate);
	 
}

function needAttentionWrapper (InDate,InDateTxt){
	
	if(InDate && InDateTxt){
		return (needAttention(InDate))?{text:InDateTxt + "*",style:'attention',attention:true}:{text:InDateTxt};
...
function printDVP2(company,vehicleList,onlyAttention){
...
for (i = 0; i < vehicleList.length; i++) { 
	...
	tax_exp_dt_txt = needAttentionWrapper(vehicleList[i].tax_exp_dt,vehicleList[i].tax_exp_dt_txt);
	alert1 = (tax_exp_dt_txt.attention)?true:false;
	npa_exp_dt_txt = needAttentionWrapper(vehicleList[i].npa_exp_dt,vehicleList[i].npa_exp_dt_txt);
	alert2 = (npa_exp_dt_txt.attention)?true:false;
	npb_exp_dt_txt = needAttentionWrapper(vehicleList[i].npb_exp_dt,vehicleList[i].npb_exp_dt_txt);
	alert3 = (npb_exp_dt_txt.attention)?true:false;
	ft_exp_dt_txt = needAttentionWrapper(vehicleList[i].ft_exp_dt,vehicleList[i].ft_exp_dt_txt);
	alert4 = (ft_exp_dt_txt.attention)?true:false;
	tp_exp_dt_txt = needAttentionWrapper(vehicleList[i].tp_exp_dt,vehicleList[i].tp_exp_dt_txt);
...
var docDefinition = {
  ...
  {text:company.company_name,style: 'header', alignment: 'center'},
