...
$scope.registerVehicle = function(){
	...
	vehicleFactory.registerVehicle($scope.vehicle, function(err,data){
	 ...
	 if(data.status != "S"){
	$scope.editMode = true;
		$scope.errMsg = 'ERROR - ' + JSON.stringify(data.status);
		$scope.showErrMsg = true;
		var temp = [];
	    if(data.msg == 'DUPLICATE_CHASSIS_ENGINE'){
...
$scope.saveTaxPayment = function(){
	...
	taxPaymentFactory.saveTaxPayment ($scope.taxPaymentCurrent,function(err,data){
        ...
        if(data.status != "S"){
        	...
        	$scope.modalTaxPaymentErrorMsg = data.msg;
...
$scope.nationalPermitCurrent.from_date = new Date();
